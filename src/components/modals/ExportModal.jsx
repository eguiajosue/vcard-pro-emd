...
function loadHtml2Pdf() {
  if (document.querySelector(`script[src="${HTML2PDF_SRC}"]`)) return;
...
export default function ExportModal() {
  const { state, activeModal, closeModal } = useApp();
  const showToast = useToast();
  const open = activeModal === 'export';

  useEffect(() => { loadHtml2Pdf(); }, []);

  if (!open) return null;

  const buildName = ext => {
    const p = [state.form.nombre.trim(), state.form.apellidos.trim()].filter(Boolean).map(slugify).join('-');
...
${d.web ? `<div class="item"><div class="ico"><i class="fa-solid fa-globe"></i></div><div class="info"><div class="lbl">Web</div><div class="val"><a href="${d.web}" target="_blank">${d.web}</a></div></div></div>` : ''}
  ...
  const exportAsPDF = () => {
    if (typeof window.html2pdf === 'undefined') { showToast('Libreria PDF no disponible. Reintenta en un momento.', 'warn'); return; }
    const d = collectFormData(state.form);
    ...
    window.html2pdf().set({ margin: 0, filename: buildName('pdf'), html2canvas: { scale: 2 }, jsPDF: { format: [85, 55], unit: 'mm', orientation: 'landscape' } }).from(el).save();
    showToast('PDF descargado.', 'success');
  };

  const OPTIONS = [
    { id: 'vcf', icon: 'fa-file-csv', label: '.VCF', desc: 'iPhone, Outlook, Gmail', run: exportAsVCF },
    { id: 'pdf', icon: 'fa-file-pdf', label: '.PDF', desc: 'Tarjeta 85x55mm para imprimir', run: exportAsPDF },
    { id: 'html', icon: 'fa-globe', label: '.HTML', desc: 'Pagina web compartible como link', run: exportAsHTML },
    { id: 'csv', icon: 'fa-table', label: '.CSV', desc: 'Excel, Google Contacts, Outlook', run: exportAsCSV },
  ];

  return (
    <div className="modal-overlay open" onClick={e => { if (e.target === e.currentTarget) closeModal(); }}>
      <div className="modal-box">
        <div className="modal-hd"><h3><i className="fa-solid fa-download"></i> Exportar Tarjeta</h3><button className="modal-close" onClick={closeModal}><i className="fa-solid fa-xmark"></i></button></div>
        <p className="modal-desc">Elige el formato que necesitas.</p>
        <div className="exp-opts">
          {OPTIONS.map(o => (
            <button key={o.id} className="exp-opt" onClick={() => { o.run(); closeModal(); }}>
